...
import { peekAdminDashboard } from '../../services/adminDashboardCache';
...
export function AdminBlockedTimesScreen() {
  const { t } = useTranslation();
  const { pickerLocale } = useAppLocale();
  const navigation = useNavigation<{ navigate: (name: string) => void; openDrawer?: () => void }>();
  const route = useRoute<{ key: string; name: string; params?: { staffId?: string } }>();
  const { token, role } = useAuth();
  const insets = useSafeAreaInsets();
  const { width: windowWidth } = useWindowDimensions();

  const warmStaffList = peekAdminDashboard(token)?.staffList ?? [];
  const [staffOptions, setStaffOptions] = useState<{ id: string; name: string }[]>(warmStaffList);
  const [staffId, setStaffId] = useState<string | null>(
    route.params?.staffId ?? warmStaffList[0]?.id ?? null,
  );
  const blocked = useAdminBlockedSlots(token, staffId);
  ...
  /**
   * The iOS spinner's own onChange fires on every scroll tick. Feeding that straight back into
   * startTime/endTime makes `value` a controlled prop that's recomputed mid-drag on every re-render —
   * a known datetimepicker footgun that causes the picker to visually settle 1 minute off from what
   * was actually dragged to. Track the in-progress pick separately and only commit on "Done".
   */
  const [draftTime, setDraftTime] = useState<Date>(() => timeHHMMToReferenceDate(''));

  useEffect(() => {
    if (!token || !canAccessAdmin(role)) return;
    let cancelled = false;
    void (async () => {
      try {
        ...
        setStaffId((prev) => prev ?? staff[0]?.id ?? null);
      } catch {
  ...
  const openTimePicker = (kind: 'start' | 'end') => {
    // Seed the draft from whatever is already committed (or 09:00 if unset) — the spinner then
    // owns its own value entirely until "Done" explicitly commits it.
    ...
    <Screen style={styles.container} noPadding>
      ...
      >
        ...
        <View style={styles.addCard}>
          <Text style={styles.label}>{t('admin.labelStaff')}</Text>
...
const styles = StyleSheet.create({
  ...
  staffChipText: { fontSize: 14, fontWeight: '600', color: colors.text },
  staffChipTextActive: { color: colors.surface },
  pickerField: {
    flexDirection: 'row-reverse',
    alignItems: 'center',
    gap: spacing.sm,
