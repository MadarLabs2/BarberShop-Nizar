-- Fix: admin_customers_list required p.api_token IS NOT NULL, which hid every customer
-- the moment they logged out. `profiles` rows are only ever created via the OTP-verify
-- registration flow, so any non-admin, non-staff-linked row is already a real customer —
-- the session-token check was never a valid proxy for "is registered" and only caused
-- logged-out customers to silently disappear from the admin customer list.
